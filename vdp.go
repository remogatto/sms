...
func (vdp *vdp) rasterizeBackground(lineAddr int, pixelOffset int, tileData int, tileDef int) {
...
func (vdp *vdp) rasterizeLine(line int) {
	...
	for i := 0; i < 32; i++ {
		...
		vdp.clearBackground(lineAddr, int(pixelOffset))
		...
		for j := 0; j < 8; j++ {
			...
			for k := 0; k < len(sprites); k++ {
				...
				spriteAddr := int(spriteBase) + int(sprite[1])*32 + spriteLine*4
